...
class AFMResNet(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, end2end=True):
        ...
        weight
        """
        self.fc_a = nn.Linear(512 * block.expansion, int(384 * block.expansion/4))
        self.fc_b = nn.Linear(512 * block.expansion, int(384 * block.expansion/4))
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            x = self.fc_classifier(mixup_x)
            naive_x = self.fc_classifier(naive_x)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    model = AFMResNet(block, layers, **kwargs)
    if pretrained:
